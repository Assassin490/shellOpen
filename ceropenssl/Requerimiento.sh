		...
		if [[ "$longitud" == "2048" ]] || [[ "$longitud" == "4096" ]]; then
			...
			if [[ -z ${cn} ]] || [[ -z ${pass} ]] ;	then
				...
				do
					...
					#openssl req -new -newkey rsa:"$longitud" -out "$cn".crt -keyout "$cn".pem  -days 365 -passin pass:"$pass" -passout pass:"$pass"
					echo -e "\n"
					read -p "¿Sus datos son correctos? 1[no], 2[si] " contador
						...
						case $contador in
							...
							2)
								...
								echo -ne '\e[1;32m####################      (100%)\e[0m\r'
								echo -ne '\n\n'
								openssl pkcs12 -export -out "$cn".pfx -inkey "$cn".pem -in "$cn".csr
								contador=2
								ls -l 
								echo -e "\n"
							;;
